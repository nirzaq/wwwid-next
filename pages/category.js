...
export default class Category extends Component {
  static async getInitialProps(context) {
    ...
    const filterByCategory = articles.filter( item => {
      if (item.categories.includes(cat)) return item;
    })

    return {
      articles: filterByCategory
    };
  }
  ...
  render() {
    return (
      <div>
        <Head>
          <title>{this.props.url.query.cat.replace("-"," ")}</title>
        </Head>
        <Header />
        <Container>
